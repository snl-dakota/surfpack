...
borehole sites\cite{cressie}. Building a Kriging
...
\end{enumerate}
The Surfpack 1.X Kriging model also allows gradient information to be used
to construct the emulator.  The default is to construct a Kriging model 
without using derivative information but Gradient Enhanced Kriging (GEK) can 
be specified by using the \verb1derivative_order1 keyword followed by the 
value ``1'' to indicate first order derivative (gradient) information.  
However, the SurfPack Data ``.spd'' file format used outside of the new 
Kriging model (nkm) directory does not support the specification of 
derivative data, so this option is not available in stand-alone compilations 
of Surfpack.  GEK is available for use when Surfpack is called by Sandia's 
DAKOTA software package for Optimization and Uncertainty Quantification.  
Directly linking to nkm object files can also enable the use of GEK because 
nkm::SurfData class is capable of reading/writing derivatives from/to a 
.spd file.  However, the use of the nkm::SurfData class has not yet been 
promoted to the rest of Surfpack.\newline
...
Kriging \cite{matheron1971theory} uses a general polynomial trend model 
...
function is a main effects (no interaction terms) quadratic polynomial.  
If the \verb1order1 keyword is used to specify a different polynomial 
order (any non negative integer is valid input for \verb1order1) then a 
full polynomial with be used unless the \verb1reduced_polynomial1 keyword 
is also used to indicate a main effects polynomial. If the amount of build
data is insufficient for the requested polynomial order it will be reduced 
automatically.\newline
...
The powered-exponential correlation function with $1\le\gamma\le2$ and 
Matern correlation function with $\nu\in\{1/2,3/2,5/2,\infty\}$ are
implemented in the Surfpack Kriging model.  The powered-exponential 
correlation function can be specified with \verb1powered_exponential1 keyword
followed a real number between 1 and 2 (inclusive).  The Matern correlation 
function can be specified with the \verb1matern1 keyword followed by 
0.5 or 1.5 or 2.5 or \verb1infinity1.  For Gradient Enhanced Kriging only
the Gaussian, Matern 3/2, and Matern 5/2 correlation functions can be used.
In tests, the Gaussian correlation function was often the most accurate (for
both Kriging and Gradient Enhanced Kriging) and as such it is the default in
Surfpack.  Its infinite smoothness or differentiability is beneficial  
for leveraging sparse data (which is the typical case).\newline
...
have physical meaning. For the powered-exponential correlation function
\begin{equation}
\theta_k=\frac{1}{\gamma\ L_k^\gamma}.
\end{equation}
For the Matern function
\begin{equation}
\theta_k=\frac{\sqrt{2\ \nu}}{L_k}.
\end{equation}
The set of correlation lengths used to construct the Surfpack Kriging Model 
can be directly specified by using \verb1optimization_method1 = \verb1none1 
and the keyword \verb1correlation_lengths1 followed a list of $M$ correlation 
lengths, where $M$ is the number of dimensions.\newline
...
\item Exactly calculating the minimum nugget needed for a target
      ...
      what is added as need when the \verb1find_nugget1 option of the 
...
The other, more common, approach to 
constructing a Kriging model involves using optimization to find the 
...
models that have different numbers of sample points, this will be important
later.\newline

In the Surfpack Kriging model, the domain of correlation length space over
which the objective function is optimized is $d/4 \le L_k \le 8d$ where
$d$ is the average distance between points; $d=N^{-1/M}$ when the input space
has be normalized to a unit hypercube or unit hype-rectangle (centered at 
zero).  Here $M$ is the number of input variables.  In Surfpack this 
normalization is always done because it make the definition of $d$ simple 
and significantly improves the conditioning of 
$\left(\underline{\underline{G}}\ \underline{\underline{R}}^{-1} \underline{\underline{G}}^T\right)$. 
The user can use the \verb1lower_bounds1 and \verb1upper_bounds1 keywords
(each of these keywords is followed by a list of $M$ real numbers) to specify
the size of the input space.  If bounds are not specified the minimum and 
maximum values of the coordinates of build data points in each dimension 
are used. The \verb1dimension_groups1 keyword can be used to specify $M$ 
integers to indicate which group each input belongs to, if this is done
then relative distance within each group is preserved when the build data is
scaled, i.e. the scaled inputs space will be a unit hyper-rectangle rather 
than a unit hypercube.  By default each dimension is considered to have its 
own group.\newline
...
\end{itemize}
The optimization used to determine $\underline{\theta}$ is performed under the
constraint that $\underline{\underline{R}}$ is not ill conditioned, 
specifically that $2^{-40}<{\rm rcond}\left(\underline{\underline{R}}\right)$. 
As indicated above, the keywords \verb1nugget1 and \verb1find_nugget1 can be 
used to avoid ill-conditioning and enable a larger set of $\underline{\theta}$ 
...
Because the Kriging $\underline{\underline{R}}$ matrix has all ones on its
diagonal the first point listed (index zero) will always be retained when
pivoted Cholesky is used to select an optimal subset of points from which
to build the Kriging Model.  The user can directly specify which point to
use/retain as the ``anchor point'' via the \verb1anchor_index1 keyword 
followed by a integer that is greater than or equal to zero and less than 
$N$.
