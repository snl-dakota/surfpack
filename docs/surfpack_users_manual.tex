...
the nkm::SurfData class is capable of reading/writing derivatives from/to a 
...
has be normalized to a unit hypercube or unit hyper-rectangle (centered at 
...
the size of the input space.  If bounds are not specified, then the minimum 
and maximum values of the coordinates of build data points in each dimension 
...
then the distance aspect ratios within each group is preserved when the build 
data is scaled, i.e. the scaled inputs space will be a unit hyper-rectangle 
rather than a unit hypercube.  By default each dimension is considered to have 
its own group.\newline
