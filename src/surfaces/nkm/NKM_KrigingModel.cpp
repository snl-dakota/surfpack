...
void KrigingModel::create()
{
  ...
  Rinv__dR_dthetai_Rinv_eps_minus_deps_dthetai.clear(); //matrix used to compute hessObj
...
void KrigingModel::masterObjectiveAndConstraints(const MtxDbl& theta, int obj_der_mode, int con_der_mode)
{
  ...
  //member variable: MtxDbl Rinv__dR_dthetai_Rinv_eps_minus_deps_dthetai(numPoints,numTheta)
  ...
  if((prevObjDerMode==0)&&(1<=obj_der_mode)) {
    ...
    RChol.copy(R);
    chol_info=0;
    Chol_fact(RChol,chol_info); 
    //printf("Chol\n");
    //assert(chol_info==0);  //here for debug, decide what to do about it later

    rcondR=rcond_after_Chol_fact(R,RChol);
    //RLU.copy(R); 
    //ipvt_RLU.newSize(numPoints);
    //LU_fact(RLU,ipvt_RLU); 
    //rcondR=rcond_after_LU_fact(R, RLU); //once we have RLU this is virtually free so have
    //it on hand/ready to use as a diagnostic in case we need it

    double log_determinant_R = 0.0; //need to do this to avoid underflow error for large numbers of points, log(0)=-inf
    //double min_fabs_RLU_diag=DBL_MAX;
    //int iptmin=0;
    for (int ipt = 0; ipt < numPoints; ++ipt) {
      log_determinant_R += log(RChol(ipt,ipt)); 
      //log_determinant_R += log(fabs(RLU(ipt,ipt))); 
      //      if(min_fabs_RLU_diag>fabs(RLU(ipt,ipt))){
      //min_fabs_RLU_diag=fabs(RLU(ipt,ipt));
      //iptmin=ipt;
      //}
    }
  ...
  //if(determinant_R==0.0) 
  //printf("[determinant_R=%g iptmin=%d min_fabs_RLU_diag=%g]",determinant_R,iptmin,min_fabs_RLU_diag);
    ...
    Chol_fact(Gtran_Rinv_G_Chol,chol_info);
    //if(~(chol_info==0)) assert(chol_info==0);  //for debug, do something else for production
    //Gtran_Rinv_G_LU.newSize(ntrend,ntrend);  
    //ipvt_Gtran_Rinv_G_LU.newSize(ntrend);
    //matrix_mult(Gtran_Rinv_G_LU,G,Rinv_G,0.0,1.0,'T','N');
    //LU_fact(Gtran_Rinv_G_LU,ipvt_Gtran_Rinv_G_LU);
    temp.newSize(ntrend);
    matrix_mult(temp, Rinv_G, Y, 0.0, 1.0, 'T', 'N');
    betaHat.newSize(ntrend);
    solve_after_Chol_fact(betaHat,Gtran_Rinv_G_Chol,temp);
    //solve_after_LU_fact(betaHat, Gtran_Rinv_G_LU, ipvt_Gtran_Rinv_G_LU, temp, 'N', 'N'); //O(ntrend^2) ops
    ...
    //solve_after_LU_fact(rhs,RLU,ipvt_RLU,temp2,'N','N'); //rhs=R^-1*(Y-G(XR)*betaHat) O(numPoints^2) ops

    //see KRD's "Koehler and Owen comments" about estVariance in 
    //KrigingProblem:objective(MtxDbl& theta)
    estVarianceMLE = dot_product(temp2,rhs)/numPoints; //the literature way

    //it's actually the log likelihood, which we want to maximize
    //double likelihood = -0.5*(numPoints*(log(4.0*acos(0.0))+log(estVarianceMLE)+1)
    //			    +log(determinant_R)); //from Koehler and Owen 
    //likelihood = -0.5*(log(4.0*acos(0.0))+log(estVarianceMLE)+1.0
    //	       +log(determinant_R)/numPoints); //on a per point basis

    //likelihood = -0.5*(log(estVarianceMLE)+log(determinant_R)/numPoints); //on a per point basis
    likelihood = -0.5*(log(estVarianceMLE)+log_determinant_R/numPoints); //on a per point basis
  ...
  if((prevObjDerMode<4)&&(4<=obj_der_mode)) {
    ...
    //consider this to be a temp variable, we do not need to retain its state
    //between sequential calls to this function
    Rinv__dR_dthetai_Rinv_eps_minus_deps_dthetai.newSize(numPoints,numTheta);
    ...
    //the j column of Rinv__dR_dthetai_Rinv_eps_minus_deps_dthetai, if we had 
    ...
    solve_after_Chol_fact(Rinv__dR_dthetai_Rinv_eps_minus_deps_dthetai,RChol,
			  dR_dthetai_Rinv_eps_minus_deps_dthetai);
    //solve_after_LU_fact(Rinv__dR_dthetai_Rinv_eps_minus_deps_dthetai,RLU,
    ...
    dcorrMtx_dthetak(dR_dthetai, R, i);
    apply_nugget_eval(dR_dthetai); //apply nugget EVAL is correct, the additive term is constant with respect to theta so drops out
    Rinv__dR_dthetai_Rinv_eps_minus_deps_dthetai.getCols(temp2,i);
    ...
    for(i=0; i<numTheta; ++i) {
      ...
      //Rinv__dR_dthetai_Rinv_eps_minus_deps_dthetai.getCols(temp3,j); //for i!=j
