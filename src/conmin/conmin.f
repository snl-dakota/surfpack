...
c      - removed the use of the common block CNMN1 by moving the
c        variables in the block to subroutine arguments, this was
c        done in each conmin subroutine that used CNMN1
c        A.A.Giunta, 03 October 2001
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      implicit           double precision(a-h,o-z)
c
c      COMMON /CNMN1/ DELFUN,DABFUN,FDCH,FDCHM,CT,CTMIN,CTL,CTLMIN,ALPHAX
c     1,ABOBJ1,THETA,OBJ,NDV,NCON,NSIDE,IPRINT,NFDG,NSCAL,LINOBJ,ITMAX,IT
c     2RM,ICNDIR,IGOTO,NAC,INFO,INFOG,ITER
C
      DIMENSION X(N1), VLB(N1), VUB(N1), G(N2), SCAL(N1), DF(N1), 
     . A(N1,N3), S(N1), G1(N2), G2(N2), B(N3,N3), C(N4), ISC(N2), 
     . IC(N3), MS1(N5)

ccc      dimension NCAL(2),ISPACE(2)

      COMMON /CONSAV/ DM1,DM2,DM3,DM4,DM5,DM6,DM7,DM8,DM9,DM10,DM11,
     . DM12,DCT,DCTL,PHI,ABOBJ,CTA,CTAM,CTBM,OBJ1,SLOPE,DX,DX1,FI,XI,
     . DFTDF1,ALP,FFF,A1,A2,A3,A4,F1,F2,F3,F4,CV1,CV2,CV3,CV4,APP,
     . ALPCA,ALPFES,ALPLN,ALPMIN,ALPNC,ALPSAV,ALPSID,ALPTOT,RSPACE,
     . IDM1,IDM2,IDM3,JDIR,IOBJ,KOBJ,KCOUNT,NCAL(2),NFEAS,MSCAL,NCOBJ,
     . NVC,KOUNT,ICOUNT,IGOOD1,
...
C     ------------------------------------------------------------------
      ...
      WRITE (6,1240) IPRINT,NDV,ITMAX,NCON,NSIDE,ICNDIR,NSCAL,NFDG,
...
680   CONTINUE
      JGOTO=0 
690   CONTINUE
      CALL CNMN06 (X,VLB,VUB,G,SCAL,DF,S,G1,G2,CTAM,CTBM,SLOPE,ALP,A2,
     . A3,A4,F1,F2,F3,CV1,CV2,CV3,CV4,ALPCA,ALPFES,ALPLN,ALPMIN,ALPNC,
     . ALPSAV,ALPSID,ALPTOT,ISC,N1,N2,NCAL,NVC,ICOUNT,IGOOD1,IGOOD2,
...
1540  FORMAT (/5X,40HGRADIENTS OF CONSTRAINTS WERE CALCULATED,I5,2X,5HTI
     1MES)
      ...
      SUBROUTINE CNMN01 (JGOTO,X,DF,G,ISC,IC,A,G1,VLB,VUB,SCAL,C,NCAL,
     . DX,DX1,FI,XI,III,N1,N2,N3,N4,
     . DELFUN,DABFUN,FDCH,FDCHM,CT,CTMIN,CTL,CTLMIN,ALPHAX,
     . ABOBJ1,THETA,OBJ,NDV,NCON,NSIDE,IPRINT,NFDG,NSCAL,LINOBJ,ITMAX,
     . ITRM,ICNDIR,IGOTO,NAC,INFO,INFOG,ITER ) 
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
180   FORMAT (//5X,46H* * DIRECTION FINDING PROCESS DID NOT CONVERGE/5X,
     129H* * S-VECTOR MAY NOT BE VALID) 
      ...
      SUBROUTINE CNMN06 (X,VLB,VUB,G,SCAL,DF,S,G1,G2,CTAM,CTBM,SLOPE,
     . ALP,A2,A3,A4,F1,F2,F3,CV1,CV2,CV3,CV4,ALPCA,ALPFES,ALPLN,ALPMIN,
